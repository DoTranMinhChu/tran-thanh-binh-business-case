  ...
  return (
    <>
      <nav className="relative shadow-md shadow-gray-400 ">
          ...
          <div className="relative flex items-center h-16">
            <div className="absolute inset-y-0 left-0 flex items-center xl:hidden">
              <button
                ...
                className="inline-flex items-center p-2 text-black border-2 border-black rounded-md hover:bg-gray-900 hover:text-gray-300 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white"
            ...
            <div className="flex items-center justify-center flex-1 xl:items-stretch xl:justify-between ">
              <div className="flex items-center flex-shrink-0">
                <Link href="/" className="px-1 py-2 text-xl font-medium ">
                  Business Case
                </Link>
              </div>
        ...
        <div
          className={`duration-500 ease-in-out bg-gray-200 top-full left-0 right-0 h-fit overflow-hidden z-0  ${
            openMobileMenu ? "xl:!-top-96" : "!-top-96"
          } absolute shadow-md shadow-gray-400`}
          id="mobile-menu"
        >
          <div className="px-2 pt-2 pb-3">
            {menuNavbarList.map((item: IItemNavbar) => {
              return (
                <ItemMenuButton
                  href={item.slug}
                  className="block px-3 py-2 text-xl font-medium"
                  active={router.asPath == item.slug}
                  key={item.name}
                >
                  {item.title}
